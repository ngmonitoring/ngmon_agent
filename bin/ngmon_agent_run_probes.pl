...
use lib $basedir.'/..'; # NGMon dir is one level higher than sbin
use NGMon::Conf; # introduces the $conf variable ...
use NGMon::RessourceManager;
use NGMon::Probe;
...
NGMon::Conf::reload();
my $resman = NGMon::RessourceManager->new();
...
sub basic_loop {
  if ($resman->can_run_new_probe()) {
    my $queue = $resman->get_queue();
    if (scalar @$queue) {
      foreach my $probe_request (@$queue) {
        if($resman->can_run_new_probe()) {
          if(!$resman->already_running($probe_request)) {
            my $probe = NGMon::Probe->new($probe_request);
            $resman->probe_register($probe); # as the probe is registered
...
POE::Session->create(
  inline_states     => {
    ...
    sigchld     => sub { print "main sig child\n"; $_[KERNEL]->sig_handled(); }, # reap children processes
    sighup      => \&sighup,
